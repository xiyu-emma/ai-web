        ...
        try:
            ...
            if os.path.exists(best_model_path):
                try:
                    ...
                    try:
                        # 1. 寫入 CSV (保留原邏輯)
                        df_preds = pd.DataFrame(confusion_details, columns=['filename', 'true_label', 'predicted_label', 'correct'])
                    ...
                    try:
                        ...
                        print(f"[YOLO 訓練] 成功寫入混淆矩陣與資料集 Excel: {confusion_xlsx_path}")
                    except Exception as xlsx_e:
                        print(f"[YOLO 訓練] 寫入 confusion_matrix_results.xlsx 失敗: {xlsx_e}")
